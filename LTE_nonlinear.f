C******************************
C Laplace Tidal Equations, with Bottom Drag (nonlinear)
C
...
C    the result of line 1 of Table 3, but users can easily modify it to solve
C    many other tidal dissipation problems.  Parameters most likely to
C    be modified by users are preceded by C???????????
C
...
C U,V and ETA at 0%,25%,50%,75%,100% for the last period, and two plots
...
C##############################################################################
      PARAMETER (NP1GRID = 41)        
C##############################################################################
C     NP2GRID = number of P2-grid lines                                       #
C##############################################################################
      PARAMETER (NP2GRID = 21)        
C##############################################################################
...
C##############################################################################
      PARAMETER (NSAVE = 200)          
      common/parm8z/ pi,Omega,Cd,sigma,RT,h0,alpha,e,theta0,g,mode,
     & rk2,rh2
...
C##############################################################################
C???????????
C   MINIMUM number of time steps per 10-period run (must be multiple of 200)
      NSTEPS =                                                                 
     & 400                                                            
...
C##############################################################################
      ...
      call dtdprx(tout8z,nsave,iset1,iset2,isinc)                              
      do 78769 is8z=iset1,iset2,isinc                                          
      call dtdplo(p1out8z,p2out8z,p3out8z,uout(0,0,ivara8z,ivarb8z,is8z)       
     &,np1,np2,0,3,ix8z,jy8z,0,title,vlon,vlat,umin,umax,tout8z(is8z))         
78769 continue                                                                 
C        *******CONTOUR PLOTS                                                  
...
C            ETA,ETAx,ETAy,ETAxx,ETAyy,ETAxy and (if applicable) T            #
C                                                                             #
...
C                                                                             #
C              C11*d(U)/dT + C12*d(V)/dT + C13*d(ETA)/dT = F1                 #
C              C21*d(U)/dT + C22*d(V)/dT + C23*d(ETA)/dT = F2                 #
C              C31*d(U)/dT + C32*d(V)/dT + C33*d(ETA)/dT = F3                 #
C                                                                             #
C     The parameters P1,P2 and derivatives with respect to these may also     #
C     be referenced (U1 = dU/dP1, etc):                                       #
C           U1,U2,U11,U22,U12                                                 #
...
C              X,Y,U,Ux,Uy,                                                   #
C                  V,Vx,Vy,                                                   #
C                  ETA,ETAx,ETAy and (if applicable) T                        #
C                                                                             #
...
C##############################################################################
                          if (j8z.eq.0) then                                   
C        OmegaIC BOUNDARY CONDITIONS SET (SEE IPERDC)                         
C                                                  G1 DEFINED                  
C     if (i8z.eq.    1) gd8z =                                                 
C    & [DEFAULT SELECTED, DEFINITION COMMENTED OUT]                            
C                                                  G2 DEFINED                  
...
C     + ETA,ETAx,ETAy at the output points.  If different variables are to   +#
C     + be saved (for later printing or plotting) the following functions    +#
C     + can be used to re-define the output variables:                       +#
